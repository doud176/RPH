...
class Application(tk.Tk):
    ...
    def auto_refresh_dispensation(self):
        ...
        self.after(3000, self.auto_refresh_dispensation)

    def auto_refresh_mrp(self):
        self.update_mrp_tree()
        self.after(3000, self.auto_refresh_mrp)

    def auto_refresh_generator(self):
        self.update_generator_tree()
        self.after(3000, self.auto_refresh_generator)
    ...
    def create_dispensation_tab(self):
        ...
        colonnes = ("Patient", "Heure d'injection", "Activité prescrite", "Volume à prélever",
                   "Activité à prélever", "Activité prélevée", "Heure d'expiration", "N° Seringue", "N° Préparation", "Expiration MRP", "Action")
    ...
    def actualiser_dispensation(self):
        ...
        # Calcul de l'activité volumique actuelle
        ...
        for idx, patient in enumerate(patients_filtered):
            # Calcul de l'activité volumique à l'heure d'injection
            try:
                ...
                # Calcul du volume à prélever = Activité prescrite / Activité volumique à l'heure d'injection
                volume_prelever = patient["activite"] / activite_volumique_injection if activite_volumique_injection > 0 else 0
            except ValueError:
                # Si l'heure d'injection n'est pas valide, utiliser l'activité volumique actuelle
                volume_prelever = patient["activite"] / activite_volumique if activite_volumique > 0 else 0
            
            # Calcul de l'activité à prélever = Volume à prélever * Activité volumique actuelle
            activite_a_prelever = volume_prelever * activite_volumique
            ...
            self.tree_disp.insert("", tk.END, iid=str(idx),
                                values=(patient["nom"], patient["heure"], f"{patient['activite']:.2f}",
                                        f"{volume_prelever:.2f}", f"{activite_a_prelever:.2f}",
                                        activite_prelevee, heure_exp, num_seringue, numero_prep, expiration_mrp_str, "Imprimer"),
                                tags=(tag,))
        self.save_data()
    ...
    def valider_prelèvement(self):
        ...
        volume_prelever = (patient["activite"] / effective_activity) * mrp_selected.volume_restant if effective_activity > 0 else 0
        activite_prelevee = simpledialog.askfloat("Validation", "Entrez l'activité réellement prélevée (MBq) :")
        if activite_prelevee is None:
            return
        prescrit = patient["activite"]
        if abs(activite_prelevee - prescrit) > 0.1 * prescrit:
            messagebox.showerror("Erreur", "L'activité prélevée diffère de plus de ±10% par rapport à l'activité prescrite.")
            return
    ...
    def on_disp_double_click(self, event):
        ...
        if region != "cell" or self.tree_disp.identify_column(event.x) != "#9":  # Colonne "Action"
            return
    ...
    def imprimer_etiquette(self, patient, mrp):
        ...
        lignes = [
            ...
            f"Act. Prélevée : {patient.get('activite_prelevee', 'N/A')} MBq",
            f"Date exp. : {mrp.creation_time.strftime('%Y-%m-%d')}",
            f"Heure Exp. : {patient.get('heure_expiration', 'N/A')}",
